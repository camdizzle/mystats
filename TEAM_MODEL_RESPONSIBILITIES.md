...
## Phase 1: Local MyTeams (single channel)
...
- Add desktop MyTeams management UI tab (settings, roster visibility, moderation helpers).
- Add team points mode toggle:
  - `season`: use full season contribution.
  - `active`: only count points while team membership is active.

## Phase 2: Global MyTeams (cross-channel)
...
- Includes bonus-window summary when active (multiplier + bonus races during active window).
...
## `!recruiting on|off`
- Captain/co-captain toggles team recruiting state in chat.
...
- For desktop/local mode, keep a persisted season cache file (for example `team_points_cache.json`) so team commands do not repeatedly rescan all historical CSV files.
- Update cache incrementally at race ingest time, including active bonus multipliers, to avoid retroactive recalculation drift.
...
- Team bonus activations should be logged with trigger amount, window start/end, and multiplier for auditability.
- Apply short command cooldowns/idempotency for mutating commands to reduce spam and duplicates.

### Team bonus model (bits-triggered)
- Streamer configures a bits threshold (example `1000`) and weighted multiplier probabilities:
  - 2x (common), 3x (mid rare), 4x (rare).
- When threshold is met for a team, activate bonus window for 20 minutes (configurable).
- During active window, all eligible team points are multiplied by active bonus multiplier.
- If another threshold is met while bonus is active, extend the window and keep the highest active multiplier.

### Subscriber points model
- Active subscribers should receive a 10% points bonus while subscribed.
- Track subscriber metadata per member so `!myteam` can surface subscriber contribution context.
